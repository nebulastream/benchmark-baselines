...
public class LRBFusedProcessorLM implements Runnable {
	...
	private final Function<HashMap<Integer, Double>, Void> sink;

	private HashMap<Integer, Double> toolsMap;

	private final AbstractHardwareSampler hwSampler;


	public LRBFusedProcessorLM(int id,
							   BufferedReader in,
							   CountDownLatch controller,
							   HashMap<Integer, Double> toolsMap,
							   int inputSize,
							   Function<HashMap<Integer, Double>, Void> sink,
			...
			for (int i = 0; i < inputSize; i++) {
				...
				if (in.getM_iType() == 0) {
					...
					if (tuple.time % 300 == 0 && tuple.time > 0 && timeOfLastTool != tuple.time) {
						...
						synchronized (LRBFusedProcessorLM.class) {
