...
public class LRBConsumer implements Runnable {


    private static final Logger LOG = LoggerFactory.getLogger(LRBConsumer.class);
        ...
        try {
            ...
            while (keepConsuming.get() > 0) {
                while (inputQueue.size() > 0) {
                    final LRBIntermediateRecord tuple = inputQueue.poll();

                    if (tuple.equals(LRBIntermediateRecord.POISONED_TUPLE)) {
